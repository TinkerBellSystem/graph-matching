...
from motif.rtm import MotifNode, MotifEdge, create_motif_node, create_alternation_node, create_group_node, \
...
def eval_function_call(caller_function_name, function_call, function_dict, motif_node_dict, name_dict):
    ...
    if function_call.name.name == "alloc_provenance":
        ...
        args = extract_function_argument_names(function_call)
        relation_type = match_relation(r_map,
                                       get_true_name(caller_function_name + '.' + args[0], name_dict).split('.')[1])
        ...
        if args[0] == 'RL_VERSION_TASK' or args[0] == 'RL_VERSION' or args[0] == 'RL_NAMED' or args[
            0] == 'RL_NAMED_PROCESS':
        ...
        args = extract_function_argument_names(function_call)
        new_motif_node = MotifNode(provenance_vertex_type(args[0]))
        return new_motif_node, None
        ...
        return eval_function_body(function_call.name.name, callee_function[1], function_dict, motif_node_dict,
                                  name_dict)
    else:
        ...
        if type(item).__name__ == 'FuncCall':  # Case 1: provenance-graph-related function call
            ...
            if type(item.expr).__name__ == 'StructRef':
                if type(item.expr.name).__name__ == 'FuncCall' and item.expr.field.name == 'version':
                    motif_node, tree_node = eval_function_call(function_name, item.expr.name, function_dict,
                                                               motif_node_dict, name_dict)
                else:
                    print(
                        '\x1b[6;30;41m[x]\x1b[0m [eval_function_body] The type {} is not implemented, or the field name is not version'.format(
...
def eval_assignment(function_name, assignment, function_dict, motif_node_dict, name_dict):
    ...
    if type(assignment.rvalue).__name__ == 'FuncCall':
        motif_node, tree_node = eval_function_call(function_name, assignment.rvalue, function_dict, motif_node_dict,
                                                   name_dict)
        # consider "var = XXX;"
        if type(assignment.lvalue).__name__ == 'ID' and get_true_name(function_name + '.' + assignment.lvalue.name,
                                                                      name_dict) in motif_node_dict:
            if motif_node is None:
                print(
                    '\x1b[6;30;41m[x]\x1b[0m [eval_assignment] Motif node {} is in the dictionary, so motif_node should not be None.'.format(
                        function_name + '.' + assignment.lvalue.name))
                raise RuntimeError('motif node should not return None type')
            else:
                motif_node_dict[get_true_name(function_name + '.' + assignment.lvalue.name, name_dict)].append(
                    motif_node)
        # consider "*var = XXX" and "&var = XXX"
        elif type(assignment.lvalue).__name__ == 'UnaryOp' and get_true_name(
                function_name + '.' + assignment.lvalue.expr.name, name_dict) in motif_node_dict:
            if motif_node is None:
                print(
                    '\x1b[6;30;41m[x]\x1b[0m [eval_assignment] Motif node {} is in the dictionary, so motif_node should not be None.'.format(
                        function_name + '.' + assignment.lvalue.expr.name))
                raise RuntimeError('motif node should not return None type')
            else:
                motif_node_dict[get_true_name(function_name + '.' + assignment.lvalue.expr.name, name_dict)].append(
                    motif_node)
        return tree_node
...
def eval_declaration(function_name, declaration, function_dict, motif_node_dict, name_dict):
    ...
    if (type(declaration.type).__name__ == 'PtrDecl' and type(declaration.type.type).__name__ == 'TypeDecl' and type(
        ...
        if type(declaration.init).__name__ == 'FuncCall':
            motif_node, tree_node = eval_function_call(function_name, declaration.init, function_dict, motif_node_dict,
                                                       name_dict)
            if motif_node is None:
        ...
        # it must be set through other methods, so we can only infer the type from its name
            ...
            if get_true_name(function_name + '.' + declaration.name, name_dict) in motif_node_dict:
                ...
                motif_node_dict[get_true_name(function_name + '.' + declaration.name, name_dict)] = [
                    create_motif_node(provenance_vertex_type(declaration.name))]
                print('\x1b[6;30;43m[!]\x1b[0m [eval_declaration] Inferring motif node from its name {}'.format(
                    declaration.name))
            return None
    else:
...
def eval_if_condition(function_name, item, function_dict, motif_node_dict, name_dict):
    ...
    if type(condition).__name__ == 'BinaryOp':
        ...
        if condition.name.name == 'vm_mayshare':
            return True
        # case: if (provenance_records_packet(prov_elt(iprov)))
        if condition.name.name == 'provenance_records_packet':
            return True
            ...
            if type(op).__name__ == 'Case' or type(op).__name__ == 'Default':
                ...
                for opstmt in opstmts:
                    if type(opstmt).__name__ == 'Assignment':
                        ...
                        motif_node, node = eval_function_call(function_name, opstmt, function_dict, motif_node_dict,
                                                              name_dict)
                        if node is not None:
                            tree_node = create_group_node(tree_node, node)
